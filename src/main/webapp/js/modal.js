...
const getMrpList = (mpsRowNode) => {
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            jsonData = JSON.parse(txt);
            console.log(jsonData);
            const result=jsonData.gridRowJson;
            mrpGridOptions.api.updateRowData({add: result.gridRowJson});
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            let resultMsg =
                ...
                + (`소요량취합 일련번호 : <br>`
                + txt.gridRowJson.gridRowJson[0].mpsNo + ` 부터 ` +txt.gridRowJson.gridRowJson[(txt.gridRowJson.gridRowJson.length)-1].mpsNo+ ` 까지 <br>
                     총 ` + (txt.gridRowJson.gridRowJson.length-1) + `건 등록 완료.<br>`) + `<br>
                     위와 같이 작업이 처리되었습니다.`;
			...
			console.log(jsonData);
            if (jsonData.gridRowJson.errorCode != 0) {
                Swal.fire({
                    text: '데이터를 불러드리는데 오류가 발생했습니다.',
                    icon: 'error',
                });
...
const registerMrpGathering = (mrpGatheringDate, mrpNoList, mrpNoAndItemCodeList) => {
    ...
    console.log("@@@@@@json@@mrpNoList");
    console.log(mrpNoList);
    console.log(encodeURI(mrpNoList));
